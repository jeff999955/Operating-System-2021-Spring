...
bmap(struct inode *ip, uint bn)
{
  ...
  if(bn < NINDIRECT){
    int direct = NDIRECT + bn / DOUBLE,
        first = (bn % DOUBLE) / SINGLE,
        second = bn % SINGLE;
    if((addr = ip->addrs[direct]) == 0)
      ip->addrs[direct] = addr = balloc(ip->dev);
...
itrunc(struct inode *ip)
{
  ...
  for(int n = 0; n < NDOUBLEINDIRECT; ++n)
...
namex(char *path, int nameiparent, char *name, int depth)
{
  ...
  while((path = skipelem(path, name)) != 0){
    ...
    if(ip->type == T_SYMLINK){
      char realpath[MAXPATH], buf[MAXPATH];
      if(readi(ip, 0, (uint64)realpath, 0, MAXPATH) == 0){
        return 0;
      }
      iunlockput(ip);
      ip = namex(realpath, 0, buf, depth + 1);
      ilock(ip);
    }
